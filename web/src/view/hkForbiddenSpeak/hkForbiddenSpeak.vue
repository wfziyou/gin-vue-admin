<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="禁言原因_编号" prop="reasonId" width="120" />
        <el-table-column align="left" label="当前状态：0 未解锁、1已解锁" prop="curStatus" width="120" />
        <el-table-column align="left" label="解锁用户_编号" prop="unlockUserId" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkForbiddenSpeakFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="禁言原因_编号:"  prop="reasonId" >
          <el-input v-model.number="formData.reasonId" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="当前状态：0 未解锁、1已解锁:"  prop="curStatus" >
          <el-input v-model.number="formData.curStatus" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="解锁用户_编号:"  prop="unlockUserId" >
          <el-input v-model.number="formData.unlockUserId" :clearable="true" placeholder="请输入" />
        </el-form-item>
...
<script>
export default {
  name: 'HkForbiddenSpeak'
}
</script>

<script setup>
import {
  createHkForbiddenSpeak,
  deleteHkForbiddenSpeak,
  deleteHkForbiddenSpeakByIds,
  updateHkForbiddenSpeak,
  findHkForbiddenSpeak,
  getHkForbiddenSpeakList
} from '@/api/hkForbiddenSpeak'
...
const getTableData = async() => {
  const table = await getHkForbiddenSpeakList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkForbiddenSpeakFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkForbiddenSpeakByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkForbiddenSpeakFunc = async(row) => {
    const res = await findHkForbiddenSpeak({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkForbiddenSpeak
...
// 删除行
const deleteHkForbiddenSpeakFunc = async (row) => {
    const res = await deleteHkForbiddenSpeak({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkForbiddenSpeak(formData.value)
                  break
                case 'update':
                  res = await updateHkForbiddenSpeak(formData.value)
                  break
                default:
                  res = await createHkForbiddenSpeak(formData.value)
                  break
