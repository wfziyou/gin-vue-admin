  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkFeedbackTypeFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createHkFeedbackType,
  deleteHkFeedbackType,
  deleteHkFeedbackTypeByIds,
  updateHkFeedbackType,
  findHkFeedbackType,
  getHkFeedbackTypeList
} from '@/api/hkFeedbackType'
...
const getTableData = async() => {
  const table = await getHkFeedbackTypeList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkFeedbackTypeByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkFeedbackTypeFunc = async(row) => {
    const res = await findHkFeedbackType({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkFeedbackType
...
// 删除行
const deleteHkFeedbackTypeFunc = async (row) => {
    const res = await deleteHkFeedbackType({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkFeedbackType(formData.value)
                  break
                case 'update':
                  res = await updateHkFeedbackType(formData.value)
                  break
                default:
                  res = await createHkFeedbackType(formData.value)
                  break
