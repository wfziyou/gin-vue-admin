...
// Author [piexlmax](https://github.com/piexlmax)
func (hkCircleUserService *HkCircleUserService) AppGetHkCircleUserInfoList(info appReq.CircleUserSearch) (list []community.HkCircleUser, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// 创建db
	db := global.GVA_DB.Model(&community.HkCircleUser{})
