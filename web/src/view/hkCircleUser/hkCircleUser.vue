<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="用户的圈子排序" prop="sort" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkCircleUserFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getHkCircleUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkCircleUserByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkCircleUserFunc = async(row) => {
    const res = await findHkCircleUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkCircleUserFunc = async (row) => {
    const res = await deleteHkCircleUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
