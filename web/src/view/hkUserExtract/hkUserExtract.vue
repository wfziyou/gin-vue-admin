<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="名称" prop="realName" width="120" />
        <el-table-column align="left" label="bank = 银行卡 alipay = 支付宝wx=微信" prop="extractType" width="120" />
        <el-table-column align="left" label="银行卡" prop="bankCode" width="120" />
        <el-table-column align="left" label="开户地址" prop="bankAddress" width="120" />
        <el-table-column align="left" label="支付宝账号" prop="alipayCode" width="120" />
        <el-table-column align="left" label="微信号" prop="wechat" width="120" />
        <el-table-column align="left" label="提现金额" prop="extractPrice" width="120" />
        <el-table-column align="left" label="mark字段" prop="mark" width="120" />
        <el-table-column align="left" label="balance字段" prop="balance" width="120" />
        <el-table-column align="left" label="无效原因" prop="failMsg" width="120" />
         <el-table-column align="left" label="failTime字段" width="180">
            <template #default="scope">{{ formatDate(scope.row.failTime) }}</template>
         </el-table-column>
        ...
        <el-table-column align="left" label="状态:0审核中 1已提现 2未通过" prop="status" width="120" />
        <el-table-column align="left" label="是否已删除" prop="isDel" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkUserExtractFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="提现金额:"  prop="extractPrice" >
          <el-input-number v-model="formData.extractPrice"  style="width:100%" :precision="2" :clearable="true"  />
        </el-form-item>
        ...
        <el-form-item label="failTime字段:"  prop="failTime" >
          <el-date-picker v-model="formData.failTime" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
...
import {
  createHkUserExtract,
  deleteHkUserExtract,
  deleteHkUserExtractByIds,
  updateHkUserExtract,
  findHkUserExtract,
  getHkUserExtractList
...
const getTableData = async() => {
  const table = await getHkUserExtractList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkUserExtractByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkUserExtractFunc = async(row) => {
    const res = await findHkUserExtract({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkUserExtractFunc = async (row) => {
    const res = await deleteHkUserExtract({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
