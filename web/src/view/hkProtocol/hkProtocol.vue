<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="协议名称" prop="name" width="120" />
        <el-table-column align="left" label="协议说明" prop="des" width="120" />
        <el-table-column align="left" label="协议内容" prop="content" width="120" />
        <el-table-column align="left" label="所属模块/插件" prop="module" width="120" />
        <el-table-column align="left" label="显示位置" prop="pos" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkProtocolFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="协议说明:"  prop="des" >
        ...
        <el-form-item label="所属模块/插件:"  prop="module" >
          <el-input v-model="formData.module" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="显示位置:"  prop="pos" >
          <el-input v-model="formData.pos" :clearable="true"  placeholder="请输入" />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getHkProtocolList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkProtocolByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkProtocolFunc = async(row) => {
    const res = await findHkProtocol({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkProtocol
...
// 删除行
const deleteHkProtocolFunc = async (row) => {
    const res = await deleteHkProtocol({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkProtocol(formData.value)
                  break
                case 'update':
                  res = await updateHkProtocol(formData.value)
                  break
                default:
                  res = await createHkProtocol(formData.value)
                  break
