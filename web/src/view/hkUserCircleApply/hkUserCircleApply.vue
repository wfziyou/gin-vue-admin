  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkUserCircleApplyFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkUserCircleApply'
}
</script>

<script setup>
import {
  createHkUserCircleApply,
  deleteHkUserCircleApply,
  deleteHkUserCircleApplyByIds,
  updateHkUserCircleApply,
  findHkUserCircleApply,
  getHkUserCircleApplyList
} from '@/api/hkUserCircleApply'
...
const getTableData = async() => {
  const table = await getHkUserCircleApplyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkUserCircleApplyFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkUserCircleApplyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkUserCircleApplyFunc = async(row) => {
    const res = await findHkUserCircleApply({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkUserCircleApply
...
// 删除行
const deleteHkUserCircleApplyFunc = async (row) => {
    const res = await deleteHkUserCircleApply({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkUserCircleApply(formData.value)
                  break
                case 'update':
                  res = await updateHkUserCircleApply(formData.value)
                  break
                default:
                  res = await createHkUserCircleApply(formData.value)
                  break
