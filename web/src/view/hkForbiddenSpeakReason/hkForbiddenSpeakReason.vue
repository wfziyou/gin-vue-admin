<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="禁言理由" prop="reason" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkForbiddenSpeakReasonFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkForbiddenSpeakReason'
}
</script>

<script setup>
import {
  createHkForbiddenSpeakReason,
  deleteHkForbiddenSpeakReason,
  deleteHkForbiddenSpeakReasonByIds,
  updateHkForbiddenSpeakReason,
  findHkForbiddenSpeakReason,
  getHkForbiddenSpeakReasonList
} from '@/api/hkForbiddenSpeakReason'
...
const getTableData = async() => {
  const table = await getHkForbiddenSpeakReasonList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkForbiddenSpeakReasonFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkForbiddenSpeakReasonByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkForbiddenSpeakReasonFunc = async(row) => {
    const res = await findHkForbiddenSpeakReason({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkForbiddenSpeakReason
...
// 删除行
const deleteHkForbiddenSpeakReasonFunc = async (row) => {
    const res = await deleteHkForbiddenSpeakReason({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkForbiddenSpeakReason(formData.value)
                  break
                case 'update':
                  res = await updateHkForbiddenSpeakReason(formData.value)
                  break
                default:
                  res = await createHkForbiddenSpeakReason(formData.value)
                  break
