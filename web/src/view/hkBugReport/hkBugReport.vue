<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作步骤" prop="content" width="120" />
        <el-table-column align="left" label="操作步骤附件" prop="contentAttachment" width="120" />
        <el-table-column align="left" label="预期结果" prop="expectedResult" width="120" />
        <el-table-column align="left" label="实际结果" prop="actualResult" width="120" />
        <el-table-column align="left" label="实际结果附件" prop="actualResultAttachment" width="120" />
        <el-table-column align="left" label="其他信息" prop="otherInfo" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkBugReportFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getHkBugReportList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkBugReportByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkBugReportFunc = async(row) => {
    const res = await findHkBugReport({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkBugReportFunc = async (row) => {
    const res = await deleteHkBugReport({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
