<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="属性：0公开 ，1受限" prop="property" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkCircleClassifyFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="属性：0公开 ，1受限:"  prop="property" >
...
<script>
export default {
  name: 'HkCircleClassify'
}
</script>

<script setup>
import {
  createHkCircleClassify,
  deleteHkCircleClassify,
  deleteHkCircleClassifyByIds,
  updateHkCircleClassify,
  findHkCircleClassify,
  getHkCircleClassifyList
} from '@/api/hkCircleClassify'
...
const getTableData = async() => {
  const table = await getHkCircleClassifyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkCircleClassifyFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkCircleClassifyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkCircleClassifyFunc = async(row) => {
    const res = await findHkCircleClassify({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkCircleClassify
...
// 删除行
const deleteHkCircleClassifyFunc = async (row) => {
    const res = await deleteHkCircleClassify({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkCircleClassify(formData.value)
                  break
                case 'update':
                  res = await updateHkCircleClassify(formData.value)
                  break
                default:
                  res = await createHkCircleClassify(formData.value)
                  break
