  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkPlatApplyGroupFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkPlatApplyGroup'
}
</script>

<script setup>
import {
  createHkPlatApplyGroup,
  deleteHkPlatApplyGroup,
  deleteHkPlatApplyGroupByIds,
  updateHkPlatApplyGroup,
  findHkPlatApplyGroup,
  getHkPlatApplyGroupList
} from '@/api/hkPlatApplyGroup'
...
const getTableData = async() => {
  const table = await getHkPlatApplyGroupList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkPlatApplyGroupFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkPlatApplyGroupByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkPlatApplyGroupFunc = async(row) => {
    const res = await findHkPlatApplyGroup({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkPlatApplyGroup
...
// 删除行
const deleteHkPlatApplyGroupFunc = async (row) => {
    const res = await deleteHkPlatApplyGroup({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkPlatApplyGroup(formData.value)
                  break
                case 'update':
                  res = await updateHkPlatApplyGroup(formData.value)
                  break
                default:
                  res = await createHkPlatApplyGroup(formData.value)
                  break
