<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="应用分组_编号" prop="applyGroupId" width="120" />
        <el-table-column align="left" label="名称别名" prop="showName" width="120" />
        <el-table-column align="left" label="应用_编号" prop="applyId" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkCircleApplyFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="应用分组_编号:"  prop="applyGroupId" >
          <el-input v-model.number="formData.applyGroupId" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="名称别名:"  prop="showName" >
          <el-input v-model="formData.showName" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="应用_编号:"  prop="applyId" >
...
import {
  createHkCircleApply,
  deleteHkCircleApply,
  deleteHkCircleApplyByIds,
  updateHkCircleApply,
  findHkCircleApply,
  getHkCircleApplyList
} from '@/api/hkCircleApply'
...
const getTableData = async() => {
  const table = await getHkCircleApplyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkCircleApplyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkCircleApplyFunc = async(row) => {
    const res = await findHkCircleApply({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkCircleApply
...
// 删除行
const deleteHkCircleApplyFunc = async (row) => {
    const res = await deleteHkCircleApply({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkCircleApply(formData.value)
                  break
                case 'update':
                  res = await updateHkCircleApply(formData.value)
                  break
                default:
                  res = await createHkCircleApply(formData.value)
                  break
