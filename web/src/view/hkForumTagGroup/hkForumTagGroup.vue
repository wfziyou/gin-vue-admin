  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkForumTagGroupFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkForumTagGroup'
}
</script>

<script setup>
import {
  createHkForumTagGroup,
  deleteHkForumTagGroup,
  deleteHkForumTagGroupByIds,
  updateHkForumTagGroup,
  findHkForumTagGroup,
  getHkForumTagGroupList
} from '@/api/hkForumTagGroup'
...
const getTableData = async() => {
  const table = await getHkForumTagGroupList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkForumTagGroupByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkForumTagGroupFunc = async(row) => {
    const res = await findHkForumTagGroup({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkForumTagGroup
...
// 删除行
const deleteHkForumTagGroupFunc = async (row) => {
    const res = await deleteHkForumTagGroup({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkForumTagGroup(formData.value)
                  break
                case 'update':
                  res = await updateHkForumTagGroup(formData.value)
                  break
                default:
                  res = await createHkForumTagGroup(formData.value)
                  break
