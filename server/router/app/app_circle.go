...
func (router *CircleRouter) InitCircleRouter(Router *gin.RouterGroup) (R gin.IRoutes) {
	...
	var circleApi = v1.ApiGroupApp.AppApiGroup.CircleApi
	{
		...
		circleRouterWithoutRecord.GET("getSelfCircleRequestList", circleApi.GetSelfCircleRequestList)       // 分页获取用户创建圈子申请记录列表
	...
	{
		circleRouter.POST("setCircleTagSort", circleApi.SetCircleTagSort)             //(圈子管理者)设置圈子标签顺序
