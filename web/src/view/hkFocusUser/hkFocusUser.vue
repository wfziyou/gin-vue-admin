<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="关注用户ID" prop="focusUserId" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkFocusUserFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="关注用户ID:"  prop="focusUserId" >
          <el-input v-model.number="formData.focusUserId" :clearable="true" placeholder="请输入" />
        </el-form-item>
...
import {
  createHkFocusUser,
  deleteHkFocusUser,
  deleteHkFocusUserByIds,
  updateHkFocusUser,
  findHkFocusUser,
  getHkFocusUserList
} from '@/api/hkFocusUser'
...
const getTableData = async() => {
  const table = await getHkFocusUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkFocusUserByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkFocusUserFunc = async(row) => {
    const res = await findHkFocusUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkFocusUser
...
// 删除行
const deleteHkFocusUserFunc = async (row) => {
    const res = await deleteHkFocusUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkFocusUser(formData.value)
                  break
                case 'update':
                  res = await updateHkFocusUser(formData.value)
                  break
                default:
                  res = await createHkFocusUser(formData.value)
                  break
