...
// @accept application/json
// @Produce application/json
// @Param data query community.HkForumPosts true "用id查询HkForumPosts"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"查询成功"}"
// @Router /hkForumPosts/findHkForumPosts [get]
func (hkForumPostsApi *HkForumPostsApi) FindHkForumPosts(c *gin.Context) {
	var hkForumPosts community.HkForumPosts
	err := c.ShouldBindQuery(&hkForumPosts)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	if rehkForumPosts, err := hkForumPostsService.GetHkForumPosts(hkForumPosts.ID); err != nil {
