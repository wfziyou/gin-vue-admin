  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkPlatApplyFunc(scope.row)">变更</el-button>
...
import {
  createHkPlatApply,
  deleteHkPlatApply,
  deleteHkPlatApplyByIds,
  updateHkPlatApply,
  findHkPlatApply,
  getHkPlatApplyList
} from '@/api/hkPlatApply'
...
const getTableData = async() => {
  const table = await getHkPlatApplyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkPlatApplyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkPlatApplyFunc = async(row) => {
    const res = await findHkPlatApply({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkPlatApply
...
// 删除行
const deleteHkPlatApplyFunc = async (row) => {
    const res = await deleteHkPlatApply({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkPlatApply(formData.value)
                  break
                case 'update':
                  res = await updateHkPlatApply(formData.value)
                  break
                default:
                  res = await createHkPlatApply(formData.value)
                  break
