<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="平台图标" prop="icon" width="120" />
        <el-table-column align="left" label="平台的应用编号" prop="appid" width="120" />
        <el-table-column align="left" label="秘钥" prop="key" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkThirdPlatformFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteHkThirdPlatform,
  deleteHkThirdPlatformByIds,
  updateHkThirdPlatform,
  findHkThirdPlatform,
  getHkThirdPlatformList
...
const getTableData = async() => {
  const table = await getHkThirdPlatformList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkThirdPlatformByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkThirdPlatformFunc = async(row) => {
    const res = await findHkThirdPlatform({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkThirdPlatformFunc = async (row) => {
    const res = await deleteHkThirdPlatform({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
