<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="标识" prop="code" width="120" />
        <el-table-column align="left" label="固定标识(不固定:0，固定:1)" prop="fixedType" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.fixedType) }}</template>
        </el-table-column>
        <el-table-column align="left" label="类型：0系统 1自定义" prop="type" width="120">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkChannelFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="固定标识(不固定:0，固定:1):"  prop="fixedType" >
          <el-switch v-model="formData.fixedType" active-color="#13ce66" inactive-color="#ff4949" active-text="是" inactive-text="否" clearable ></el-switch>
        </el-form-item>
        <el-form-item label="类型：0系统 1自定义:"  prop="type" >
...
const onSubmit = () => {
  page.value = 1
  pageSize.value = 10
  if (searchInfo.value.fixedType === ""){
      searchInfo.value.fixedType=null
  }
...
const getTableData = async() => {
  const table = await getHkChannelList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkChannelByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkChannelFunc = async(row) => {
    const res = await findHkChannel({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkChannel
...
// 删除行
const deleteHkChannelFunc = async (row) => {
    const res = await deleteHkChannel({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkChannel(formData.value)
                  break
                case 'update':
                  res = await updateHkChannel(formData.value)
                  break
                default:
                  res = await createHkChannel(formData.value)
                  break
