<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="帖子编号" prop="postsId" width="120" />
        <el-table-column align="left" label="评论者" prop="userId" width="120" />
         <el-table-column align="left" label="评论时间" width="180">
            <template #default="scope">{{ formatDate(scope.row.commentTime) }}</template>
         </el-table-column>
        <el-table-column align="left" label="评论内容" prop="commentContent" width="120" />
        <el-table-column align="left" label="点赞数" prop="likeTimes" width="120" />
        <el-table-column align="left" label="父评论编号" prop="parentId" width="120" />
        ...
        <el-table-column align="left" label="审核状态(0未审批 1通过 2拒绝)" prop="checkStatus" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkForumCommentFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="帖子编号:"  prop="postsId" >
          <el-input v-model.number="formData.postsId" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="评论者:"  prop="userId" >
          <el-input v-model.number="formData.userId" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="评论时间:"  prop="commentTime" >
          <el-date-picker v-model="formData.commentTime" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
        <el-form-item label="评论内容:"  prop="commentContent" >
          <el-input v-model="formData.commentContent" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="点赞数:"  prop="likeTimes" >
          <el-input v-model.number="formData.likeTimes" :clearable="true" placeholder="请输入" />
        </el-form-item>
        ...
        <el-form-item label="审核状态(0未审批 1通过 2拒绝):"  prop="checkStatus" >
...
<script setup>
import {
  createHkForumComment,
  deleteHkForumComment,
  deleteHkForumCommentByIds,
  updateHkForumComment,
  findHkForumComment,
  getHkForumCommentList
} from '@/api/hkForumComment'
...
const getTableData = async() => {
  const table = await getHkForumCommentList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkForumCommentByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkForumCommentFunc = async(row) => {
    const res = await findHkForumComment({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkForumComment
...
// 删除行
const deleteHkForumCommentFunc = async (row) => {
    const res = await deleteHkForumComment({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkForumComment(formData.value)
                  break
                case 'update':
                  res = await updateHkForumComment(formData.value)
                  break
                default:
                  res = await createHkForumComment(formData.value)
                  break
