<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="拥有者：0平台、1圈子、2个人" prop="owerType" width="120" />
        <el-table-column align="left" label="圈子_编号" prop="circleId" width="120" />
        <el-table-column align="left" label="用户_编号" prop="userId" width="120" />
        ...
        <el-table-column align="left" label="类型(0小程序、1第三方链接)" prop="type" width="120" />
        <el-table-column align="left" label="小程序id" prop="miniProgramId" width="120" />
        <el-table-column align="left" label="访问地址" prop="applyAddress" width="120" />
        <el-table-column align="left" label="访问参数" prop="applyParameters" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkApplyFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="拥有者：0平台、1圈子、2个人:"  prop="owerType" >
          <el-input v-model.number="formData.owerType" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="圈子_编号:"  prop="circleId" >
          <el-input v-model.number="formData.circleId" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="用户_编号:"  prop="userId" >
        ...
        <el-form-item label="类型(0小程序、1第三方链接):"  prop="type" >
          <el-input v-model.number="formData.type" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="小程序id:"  prop="miniProgramId" >
          <el-input v-model.number="formData.miniProgramId" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="访问地址:"  prop="applyAddress" >
          <el-input v-model="formData.applyAddress" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="访问参数:"  prop="applyParameters" >
          <el-input v-model="formData.applyParameters" :clearable="true"  placeholder="请输入" />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getHkApplyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkApplyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkApplyFunc = async(row) => {
    const res = await findHkApply({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkApply
...
// 删除行
const deleteHkApplyFunc = async (row) => {
    const res = await deleteHkApply({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkApply(formData.value)
                  break
                case 'update':
                  res = await updateHkApply(formData.value)
                  break
                default:
                  res = await createHkApply(formData.value)
                  break
