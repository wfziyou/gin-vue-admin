  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="帖子编号" prop="postsId" width="120" />
         <el-table-column align="left" label="收藏时间" width="180">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkUserCollectFunc(scope.row)">变更</el-button>
...
import {
  createHkUserCollect,
  deleteHkUserCollect,
  deleteHkUserCollectByIds,
  updateHkUserCollect,
  findHkUserCollect,
  getHkUserCollectList
} from '@/api/hkUserCollect'
...
const getTableData = async() => {
  const table = await getHkUserCollectList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkUserCollectByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkUserCollectFunc = async(row) => {
    const res = await findHkUserCollect({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkUserCollect
...
// 删除行
const deleteHkUserCollectFunc = async (row) => {
    const res = await deleteHkUserCollect({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkUserCollect(formData.value)
                  break
                case 'update':
                  res = await updateHkUserCollect(formData.value)
                  break
                default:
                  res = await createHkUserCollect(formData.value)
                  break
