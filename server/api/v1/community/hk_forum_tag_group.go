...
// @accept application/json
// @Produce application/json
// @Param data query community.HkForumTagGroup true "用id查询HkForumTagGroup"
...
func (hkForumTagGroupApi *HkForumTagGroupApi) FindHkForumTagGroup(c *gin.Context) {
	var hkForumTagGroup community.HkForumTagGroup
	err := c.ShouldBindQuery(&hkForumTagGroup)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	if rehkForumTagGroup, err := hkForumTagGroupService.GetHkForumTagGroup(hkForumTagGroup.ID); err != nil {
