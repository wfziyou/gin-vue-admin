<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="用户编号" prop="uuid" width="120" />
        <el-table-column align="left" label="用户平台" prop="userType" width="120" />
        ...
        <el-table-column align="left" label="真名" prop="realName" width="120" />
        <el-table-column align="left" label="头像" prop="headerImg" width="120" />
        ...
        <el-table-column align="left" label="用户角色ID" prop="roleId" width="120" />
        ...
        <el-table-column align="left" label="状态(用户是否被冻结) 1正常 2冻结" prop="status" width="120" />
        <el-table-column align="left" label="是否已删除" prop="isDeleted" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkUserFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getHkUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkUserByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkUserFunc = async(row) => {
    const res = await findHkUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkUserFunc = async (row) => {
    const res = await deleteHkUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
