  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkUserBrowsingHistoryFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkUserBrowsingHistory'
}
</script>

<script setup>
import {
  createHkUserBrowsingHistory,
  deleteHkUserBrowsingHistory,
  deleteHkUserBrowsingHistoryByIds,
  updateHkUserBrowsingHistory,
  findHkUserBrowsingHistory,
  getHkUserBrowsingHistoryList
} from '@/api/hkUserBrowsingHistory'
...
const getTableData = async() => {
  const table = await getHkUserBrowsingHistoryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkUserBrowsingHistoryFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkUserBrowsingHistoryByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkUserBrowsingHistoryFunc = async(row) => {
    const res = await findHkUserBrowsingHistory({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkUserBrowsingHistory
...
// 删除行
const deleteHkUserBrowsingHistoryFunc = async (row) => {
    const res = await deleteHkUserBrowsingHistory({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkUserBrowsingHistory(formData.value)
                  break
                case 'update':
                  res = await updateHkUserBrowsingHistory(formData.value)
                  break
                default:
                  res = await createHkUserBrowsingHistory(formData.value)
                  break
