    ...
    <div class="gva-form-box">
      <el-form :model="formData" ref="elFormRef" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="购买赠送金额:" prop="givePrice">
          <el-input-number v-model="formData.givePrice" :precision="2" :clearable="true"></el-input-number>
        </el-form-item>
        ...
        <el-form-item label="退款金额:" prop="refundPrice">
          <el-input-number v-model="formData.refundPrice" :precision="2" :clearable="true"></el-input-number>
        </el-form-item>
...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findHkUserRecharge({ ID: route.query.id })
      if (res.code === 0) {
