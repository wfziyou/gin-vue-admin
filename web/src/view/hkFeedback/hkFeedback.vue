<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="类型编号" prop="typeId" width="120" />
        ...
        <el-table-column align="left" label="处理状态：0 未处理、1 处理中、2 拒绝、3 完成" prop="checkStatus" width="120" />
        <el-table-column align="left" label="处理描述" prop="process" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkFeedbackFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getHkFeedbackList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkFeedbackByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkFeedbackFunc = async(row) => {
    const res = await findHkFeedback({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkFeedbackFunc = async (row) => {
    const res = await deleteHkFeedback({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
