<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="类型：0官方圈子 ，1用户圈子" prop="type" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkCircleRequestFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="类型：0官方圈子 ，1用户圈子:"  prop="type" >
...
<script>
export default {
  name: 'HkCircleRequest'
}
</script>

<script setup>
import {
  createHkCircleRequest,
  deleteHkCircleRequest,
  deleteHkCircleRequestByIds,
  updateHkCircleRequest,
  findHkCircleRequest,
  getHkCircleRequestList
} from '@/api/hkCircleRequest'
...
const getTableData = async() => {
  const table = await getHkCircleRequestList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkCircleRequestByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkCircleRequestFunc = async(row) => {
    const res = await findHkCircleRequest({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkCircleRequest
...
// 删除行
const deleteHkCircleRequestFunc = async (row) => {
    const res = await deleteHkCircleRequest({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkCircleRequest(formData.value)
                  break
                case 'update':
                  res = await updateHkCircleRequest(formData.value)
                  break
                default:
                  res = await createHkCircleRequest(formData.value)
                  break
