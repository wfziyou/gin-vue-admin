<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="评论编号" prop="commentId" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkCommentThumbsUpFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkCommentThumbsUp'
}
</script>

<script setup>
import {
  createHkCommentThumbsUp,
  deleteHkCommentThumbsUp,
  deleteHkCommentThumbsUpByIds,
  updateHkCommentThumbsUp,
  findHkCommentThumbsUp,
  getHkCommentThumbsUpList
} from '@/api/hkCommentThumbsUp'
...
const getTableData = async() => {
  const table = await getHkCommentThumbsUpList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkCommentThumbsUpFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkCommentThumbsUpByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkCommentThumbsUpFunc = async(row) => {
    const res = await findHkCommentThumbsUp({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkCommentThumbsUp
...
// 删除行
const deleteHkCommentThumbsUpFunc = async (row) => {
    const res = await deleteHkCommentThumbsUp({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkCommentThumbsUp(formData.value)
                  break
                case 'update':
                  res = await updateHkCommentThumbsUp(formData.value)
                  break
                default:
                  res = await createHkCommentThumbsUp(formData.value)
                  break
