  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="用户id" prop="userId" width="120" />
         <el-table-column align="left" label="点赞时间" width="180">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkForumThumbsUpFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="点赞时间:"  prop="time" >
...
<script>
export default {
  name: 'HkForumThumbsUp'
}
</script>

<script setup>
import {
  createHkForumThumbsUp,
  deleteHkForumThumbsUp,
  deleteHkForumThumbsUpByIds,
  updateHkForumThumbsUp,
  findHkForumThumbsUp,
  getHkForumThumbsUpList
} from '@/api/hkForumThumbsUp'
...
const getTableData = async() => {
  const table = await getHkForumThumbsUpList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkForumThumbsUpByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkForumThumbsUpFunc = async(row) => {
    const res = await findHkForumThumbsUp({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkForumThumbsUp
...
// 删除行
const deleteHkForumThumbsUpFunc = async (row) => {
    const res = await deleteHkForumThumbsUp({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkForumThumbsUp(formData.value)
                  break
                case 'update':
                  res = await updateHkForumThumbsUp(formData.value)
                  break
                default:
                  res = await createHkForumThumbsUp(formData.value)
                  break
