...
// @accept application/json
// @Produce application/json
// @Param data query community.HkForumPostsGroup true "用id查询HkForumPostsGroup"
...
func (hkForumPostsGroupApi *HkForumPostsGroupApi) FindHkForumPostsGroup(c *gin.Context) {
	var hkForumPostsGroup community.HkForumPostsGroup
	err := c.ShouldBindQuery(&hkForumPostsGroup)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	if rehkForumPostsGroup, err := hkForumPostsGroupService.GetHkForumPostsGroup(hkForumPostsGroup.ID); err != nil {
