...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if token == "" {
			response.FailWithAuth(gin.H{"reload": true}, "未登录或非法访问", c)
			c.Abort()
			return
		}
		if jwtService.IsBlacklist(token) {
			response.FailWithAuth(gin.H{"reload": true}, "您的帐户异地登陆或令牌失效", c)
			c.Abort()
			return
		}
		j := jwtEx.NewJWT()
		...
		if err != nil {
			if errors.Is(err, jwtEx.TokenExpired) {
				response.FailWithAuth(gin.H{"reload": true}, "授权已过期", c)
				c.Abort()
				return
			}
			response.FailWithAuth(gin.H{"reload": true}, err.Error(), c)
			c.Abort()
			return
		}
