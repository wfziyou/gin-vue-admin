...
// @accept application/json
// @Produce application/json
// @Param data query apply.HkCircleApplyGroup true "用id查询HkCircleApplyGroup"
...
func (hkCircleApplyGroupApi *HkCircleApplyGroupApi) FindHkCircleApplyGroup(c *gin.Context) {
	var hkCircleApplyGroup apply.HkCircleApplyGroup
	err := c.ShouldBindQuery(&hkCircleApplyGroup)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	if rehkCircleApplyGroup, err := hkCircleApplyGroupService.GetHkCircleApplyGroup(hkCircleApplyGroup.ID); err != nil {
