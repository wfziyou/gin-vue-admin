...
// Author [piexlmax](https://github.com/piexlmax)
func (hkCircleService *HkCircleService) AppGetHkCircleInfoList(info appReq.CircleSearch) (list []community.HkCircle, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// 创建db
	db := global.GVA_DB.Model(&community.HkCircle{})
...
// Author [piexlmax](https://github.com/piexlmax)
func (hkCircleService *HkCircleService) GetSelfCircleList(info appReq.SelfCircleSearch) (list []community.HkCircle, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// 创建db
	db := global.GVA_DB.Model(&community.HkCircle{})
