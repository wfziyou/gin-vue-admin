  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkForumTopicGroupFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkForumTopicGroup'
}
</script>

<script setup>
import {
  createHkForumTopicGroup,
  deleteHkForumTopicGroup,
  deleteHkForumTopicGroupByIds,
  updateHkForumTopicGroup,
  findHkForumTopicGroup,
  getHkForumTopicGroupList
} from '@/api/hkForumTopicGroup'
...
const getTableData = async() => {
  const table = await getHkForumTopicGroupList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkForumTopicGroupFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkForumTopicGroupByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkForumTopicGroupFunc = async(row) => {
    const res = await findHkForumTopicGroup({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkForumTopicGroup
...
// 删除行
const deleteHkForumTopicGroupFunc = async (row) => {
    const res = await deleteHkForumTopicGroup({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkForumTopicGroup(formData.value)
                  break
                case 'update':
                  res = await updateHkForumTopicGroup(formData.value)
                  break
                default:
                  res = await createHkForumTopicGroup(formData.value)
                  break
