<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="时长" prop="count" width="120" />
        <el-table-column align="left" label="时间类型：0 小时、1天" prop="type" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkForbiddenSpeakDurationFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="时长:"  prop="count" >
          <el-input v-model.number="formData.count" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="时间类型：0 小时、1天:"  prop="type" >
...
<script>
export default {
  name: 'HkForbiddenSpeakDuration'
}
</script>

<script setup>
import {
  createHkForbiddenSpeakDuration,
  deleteHkForbiddenSpeakDuration,
  deleteHkForbiddenSpeakDurationByIds,
  updateHkForbiddenSpeakDuration,
  findHkForbiddenSpeakDuration,
  getHkForbiddenSpeakDurationList
} from '@/api/hkForbiddenSpeakDuration'
...
const getTableData = async() => {
  const table = await getHkForbiddenSpeakDurationList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkForbiddenSpeakDurationFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkForbiddenSpeakDurationByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkForbiddenSpeakDurationFunc = async(row) => {
    const res = await findHkForbiddenSpeakDuration({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkForbiddenSpeakDuration
...
// 删除行
const deleteHkForbiddenSpeakDurationFunc = async (row) => {
    const res = await deleteHkForbiddenSpeakDuration({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkForbiddenSpeakDuration(formData.value)
                  break
                case 'update':
                  res = await updateHkForbiddenSpeakDuration(formData.value)
                  break
                default:
                  res = await createHkForbiddenSpeakDuration(formData.value)
                  break
