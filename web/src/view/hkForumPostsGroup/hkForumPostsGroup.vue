<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="父节点编号" prop="parentId" width="120" />
        ...
        <el-table-column align="left" label="审核状态(0未审批 1通过 2拒绝)" prop="status" width="120" />
        <el-table-column align="left" label="是否已删除" prop="isDeleted" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkForumPostsGroupFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="父节点编号:"  prop="parentId" >
        ...
        <el-form-item label="审核状态(0未审批 1通过 2拒绝):"  prop="status" >
...
<script>
export default {
  name: 'HkForumPostsGroup'
}
</script>

<script setup>
import {
  createHkForumPostsGroup,
  deleteHkForumPostsGroup,
  deleteHkForumPostsGroupByIds,
  updateHkForumPostsGroup,
  findHkForumPostsGroup,
  getHkForumPostsGroupList
} from '@/api/hkForumPostsGroup'
...
const getTableData = async() => {
  const table = await getHkForumPostsGroupList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkForumPostsGroupFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkForumPostsGroupByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkForumPostsGroupFunc = async(row) => {
    const res = await findHkForumPostsGroup({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkForumPostsGroup
...
// 删除行
const deleteHkForumPostsGroupFunc = async (row) => {
    const res = await deleteHkForumPostsGroup({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkForumPostsGroup(formData.value)
                  break
                case 'update':
                  res = await updateHkForumPostsGroup(formData.value)
                  break
                default:
                  res = await createHkForumPostsGroup(formData.value)
                  break
