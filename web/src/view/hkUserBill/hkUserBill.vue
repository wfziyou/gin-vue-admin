<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="关联id" prop="linkId" width="120" />
        <el-table-column align="left" label="0 = 支出 1 = 获得" prop="pm" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.pm) }}</template>
        </el-table-column>
        <el-table-column align="left" label="账单标题" prop="title" width="120" />
        <el-table-column align="left" label="明细种类" prop="category" width="120" />
        <el-table-column align="left" label="明细类型" prop="type" width="120" />
        <el-table-column align="left" label="明细数字" prop="number" width="120" />
        <el-table-column align="left" label="剩余" prop="balance" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkUserBillFunc(scope.row)">变更</el-button>
...
const onSubmit = () => {
  page.value = 1
  pageSize.value = 10
  if (searchInfo.value.pm === ""){
      searchInfo.value.pm=null
  }
  getTableData()
}
...
const getTableData = async() => {
  const table = await getHkUserBillList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkUserBillByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkUserBillFunc = async(row) => {
    const res = await findHkUserBill({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkUserBillFunc = async (row) => {
    const res = await deleteHkUserBill({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
