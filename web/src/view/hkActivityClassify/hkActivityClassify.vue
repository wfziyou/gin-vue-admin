  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkActivityClassifyFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkActivityClassify'
}
</script>

<script setup>
import {
  createHkActivityClassify,
  deleteHkActivityClassify,
  deleteHkActivityClassifyByIds,
  updateHkActivityClassify,
  findHkActivityClassify,
  getHkActivityClassifyList
} from '@/api/hkActivityClassify'
...
const getTableData = async() => {
  const table = await getHkActivityClassifyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkActivityClassifyFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkActivityClassifyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkActivityClassifyFunc = async(row) => {
    const res = await findHkActivityClassify({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkActivityClassify
...
// 删除行
const deleteHkActivityClassifyFunc = async (row) => {
    const res = await deleteHkActivityClassify({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkActivityClassify(formData.value)
                  break
                case 'update':
                  res = await updateHkActivityClassify(formData.value)
                  break
                default:
                  res = await createHkActivityClassify(formData.value)
                  break
