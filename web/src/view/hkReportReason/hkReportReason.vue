<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="举报理由" prop="reason" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkReportReasonFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createHkReportReason,
  deleteHkReportReason,
  deleteHkReportReasonByIds,
  updateHkReportReason,
  findHkReportReason,
  getHkReportReasonList
} from '@/api/hkReportReason'
...
const getTableData = async() => {
  const table = await getHkReportReasonList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkReportReasonByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkReportReasonFunc = async(row) => {
    const res = await findHkReportReason({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkReportReason
...
// 删除行
const deleteHkReportReasonFunc = async (row) => {
    const res = await deleteHkReportReason({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkReportReason(formData.value)
                  break
                case 'update':
                  res = await updateHkReportReason(formData.value)
                  break
                default:
                  res = await createHkReportReason(formData.value)
                  break
