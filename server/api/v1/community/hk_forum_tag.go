...
// @accept application/json
// @Produce application/json
// @Param data query community.HkForumTag true "用id查询HkForumTag"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"查询成功"}"
// @Router /hkForumTag/findHkForumTag [get]
func (hkForumTagApi *HkForumTagApi) FindHkForumTag(c *gin.Context) {
	var hkForumTag community.HkForumTag
	err := c.ShouldBindQuery(&hkForumTag)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	if rehkForumTag, err := hkForumTagService.GetHkForumTag(hkForumTag.ID); err != nil {
