<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="签到说明" prop="title" width="120" />
        <el-table-column align="left" label="获得积分" prop="number" width="120" />
        <el-table-column align="left" label="剩余积分" prop="balance" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkUserSignFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getHkUserSignList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkUserSignByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkUserSignFunc = async(row) => {
    const res = await findHkUserSign({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkUserSign
...
// 删除行
const deleteHkUserSignFunc = async (row) => {
    const res = await deleteHkUserSign({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkUserSign(formData.value)
                  break
                case 'update':
                  res = await updateHkUserSign(formData.value)
                  break
                default:
                  res = await createHkUserSign(formData.value)
                  break
