...
func NewClient(config *config.OpenIm) (*HttpClient, error) {
	client := &HttpClient{}
	err := client.Init(config)
	return client, err
}
func (httpClient *HttpClient) Init(config *config.OpenIm) (_err error) {
	if config == nil {
		return &ErrorResponse{Message: "请配置 OpenIm"}
	}
	var baseUrl string
	if len(config.Url) > 0 {
		baseUrl = config.Url
...
func (httpClient *HttpClient) Post(name string, postData interface{}, result interface{}) (err error) {
	...
	req, err := http.NewRequest(http.MethodPost, urlStr, dataReader)
	if err != nil {
		return err
	}
