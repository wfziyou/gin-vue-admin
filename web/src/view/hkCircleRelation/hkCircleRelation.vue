<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="关系类型：0父子节点 1关注" prop="relationType" width="120" />
        <el-table-column align="left" label="圈子_编号" prop="circleId" width="120" />
        <el-table-column align="left" label="关系圈子_编号（ 关系类型0：父节点编号； 关系类型1：关注圈子编号）" prop="otherCircleId" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkCircleRelationFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="关系类型：0父子节点 1关注:"  prop="relationType" >
        ...
        <el-form-item label="关系圈子_编号（ 关系类型0：父节点编号； 关系类型1：关注圈子编号）:"  prop="otherCircleId" >
          <el-input v-model.number="formData.otherCircleId" :clearable="true" placeholder="请输入" />
        </el-form-item>
...
<script>
export default {
  name: 'HkCircleRelation'
}
</script>

<script setup>
import {
  createHkCircleRelation,
  deleteHkCircleRelation,
  deleteHkCircleRelationByIds,
  updateHkCircleRelation,
  findHkCircleRelation,
  getHkCircleRelationList
} from '@/api/hkCircleRelation'
...
const getTableData = async() => {
  const table = await getHkCircleRelationList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkCircleRelationFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkCircleRelationByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkCircleRelationFunc = async(row) => {
    const res = await findHkCircleRelation({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkCircleRelation
...
// 删除行
const deleteHkCircleRelationFunc = async (row) => {
    const res = await deleteHkCircleRelation({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkCircleRelation(formData.value)
                  break
                case 'update':
                  res = await updateHkCircleRelation(formData.value)
                  break
                default:
                  res = await createHkCircleRelation(formData.value)
                  break
