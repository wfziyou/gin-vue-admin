...
import (
	...
	"github.com/flipped-aurora/gin-vue-admin/server/pkg/common/jwt"
...
func (authApi *AuthApi) LoginTelephone(c *gin.Context) {
	...
	if err != nil {
		...
		userObj, err := UserRegister(userInfo, req.Platform)
		if err != nil {
...
func (authApi *AuthApi) Register(c *gin.Context) {
	...
	userInfo := &community.User{Account: req.Account, Password: req.Password}
	userObj, err := UserRegister(userInfo, req.Platform)
	if err != nil {
		global.GVA_LOG.Error("注册失败!", zap.Error(err))
		response.FailWithMessage(err.Error(), c)
	...
	response.OkWithDetailed(authRes.RegisterResponse{User: userObj}, "注册成功", c)
}
...
func TokenNext(c *gin.Context, user *community.User, platform int) {
	...
	clientIp := c.ClientIP()
	token, expTime, err := j.CreateTokenEx(user.ID, platform, user.AuthorityId)
	if err != nil {
...
func UserRegister(userInfo *community.User, platform int) (userInter community.User, err error) {
	userObj, err := appUserService.Register(*userInfo)
	if err != nil {
		global.GVA_LOG.Error("注册失败!", zap.Error(err))
		return
	}
	err = ImRegiser(&userObj, platform)
	if err != nil {
		return
	}
	//appCircleService.NewRegisterUserFocusDefaultCircle(userObj.ID)
	return userObj, err
}
