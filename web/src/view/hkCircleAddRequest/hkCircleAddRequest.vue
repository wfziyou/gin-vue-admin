  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkCircleAddRequestFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkCircleAddRequest'
}
</script>

<script setup>
import {
  createHkCircleAddRequest,
  deleteHkCircleAddRequest,
  deleteHkCircleAddRequestByIds,
  updateHkCircleAddRequest,
  findHkCircleAddRequest,
  getHkCircleAddRequestList
} from '@/api/hkCircleAddRequest'
...
const getTableData = async() => {
  const table = await getHkCircleAddRequestList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkCircleAddRequestFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkCircleAddRequestByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkCircleAddRequestFunc = async(row) => {
    const res = await findHkCircleAddRequest({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkCircleAddRequest
...
// 删除行
const deleteHkCircleAddRequestFunc = async (row) => {
    const res = await deleteHkCircleAddRequest({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkCircleAddRequest(formData.value)
                  break
                case 'update':
                  res = await updateHkCircleAddRequest(formData.value)
                  break
                default:
                  res = await createHkCircleAddRequest(formData.value)
                  break
