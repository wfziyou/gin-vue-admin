<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="当前版本" prop="curVersion" width="120" />
        <el-table-column align="left" label="当前包id" prop="curPacketId" width="120" />
        <el-table-column align="left" label="隐藏(0否 1是)" prop="hidden" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkMiniProgramFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="当前版本:"  prop="curVersion" >
          <el-input v-model="formData.curVersion" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="当前包id:"  prop="curPacketId" >
          <el-input v-model="formData.curPacketId" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="隐藏(0否 1是):"  prop="hidden" >
...
import {
  createHkMiniProgram,
  deleteHkMiniProgram,
  deleteHkMiniProgramByIds,
  updateHkMiniProgram,
  findHkMiniProgram,
  getHkMiniProgramList
} from '@/api/hkMiniProgram'
...
const getTableData = async() => {
  const table = await getHkMiniProgramList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkMiniProgramByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkMiniProgramFunc = async(row) => {
    const res = await findHkMiniProgram({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkMiniProgram
...
// 删除行
const deleteHkMiniProgramFunc = async (row) => {
    const res = await deleteHkMiniProgram({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkMiniProgram(formData.value)
                  break
                case 'update':
                  res = await updateHkMiniProgram(formData.value)
                  break
                default:
                  res = await createHkMiniProgram(formData.value)
                  break
