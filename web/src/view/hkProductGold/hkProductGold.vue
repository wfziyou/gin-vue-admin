<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="商品图片" prop="image" width="120" />
        <el-table-column align="left" label="数量" prop="num" width="120" />
        <el-table-column align="left" label="商品价格" prop="price" width="120" />
        <el-table-column align="left" label="会员价格" prop="vipPrice" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkProductGoldFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="会员价格:"  prop="vipPrice" >
          <el-input v-model.number="formData.vipPrice" :clearable="true" placeholder="请输入" />
        </el-form-item>
...
import {
  createHkProductGold,
  deleteHkProductGold,
  deleteHkProductGoldByIds,
  updateHkProductGold,
  findHkProductGold,
  getHkProductGoldList
} from '@/api/hkProductGold'
...
const getTableData = async() => {
  const table = await getHkProductGoldList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkProductGoldByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkProductGoldFunc = async(row) => {
    const res = await findHkProductGold({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkProductGold
...
// 删除行
const deleteHkProductGoldFunc = async (row) => {
    const res = await deleteHkProductGold({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkProductGold(formData.value)
                  break
                case 'update':
                  res = await updateHkProductGold(formData.value)
                  break
                default:
                  res = await createHkProductGold(formData.value)
                  break
