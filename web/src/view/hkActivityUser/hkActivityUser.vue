<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="活动编号" prop="activityId" width="120" />
        <el-table-column align="left" label="用户编号" prop="userId" width="120" />
        <el-table-column align="left" label="权限：0普通 1管理员 2发起者" prop="power" width="120" />
        <el-table-column align="left" label="是否付费：0 否 1 是" prop="pay" width="120" />
        <el-table-column align="left" label="订单编号" prop="orderId" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkActivityUserFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="权限：0普通 1管理员 2发起者:"  prop="power" >
...
<script setup>
import {
  createHkActivityUser,
  deleteHkActivityUser,
  deleteHkActivityUserByIds,
  updateHkActivityUser,
  findHkActivityUser,
  getHkActivityUserList
} from '@/api/hkActivityUser'
...
const getTableData = async() => {
  const table = await getHkActivityUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkActivityUserByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkActivityUserFunc = async(row) => {
    const res = await findHkActivityUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkActivityUser
...
// 删除行
const deleteHkActivityUserFunc = async (row) => {
    const res = await deleteHkActivityUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkActivityUser(formData.value)
                  break
                case 'update':
                  res = await updateHkActivityUser(formData.value)
                  break
                default:
                  res = await createHkActivityUser(formData.value)
                  break
