<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="标题图标" prop="titleIcon" width="120" />
        <el-table-column align="left" label="类型" prop="type" width="120" />
        <el-table-column align="left" label="之前金额" prop="beforeNumber" width="120" />
        <el-table-column align="left" label="改变金额" prop="changeNumber" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkGoldBillFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getHkGoldBillList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHkGoldBillByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkGoldBillFunc = async(row) => {
    const res = await findHkGoldBill({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkGoldBillFunc = async (row) => {
    const res = await deleteHkGoldBill({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
