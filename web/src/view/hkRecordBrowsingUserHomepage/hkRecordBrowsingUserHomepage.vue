  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkRecordBrowsingUserHomepageFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteHkRecordBrowsingUserHomepage,
  deleteHkRecordBrowsingUserHomepageByIds,
  updateHkRecordBrowsingUserHomepage,
  findHkRecordBrowsingUserHomepage,
  getHkRecordBrowsingUserHomepageList
...
const getTableData = async() => {
  const table = await getHkRecordBrowsingUserHomepageList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkRecordBrowsingUserHomepageFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkRecordBrowsingUserHomepageByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkRecordBrowsingUserHomepageFunc = async(row) => {
    const res = await findHkRecordBrowsingUserHomepage({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHkRecordBrowsingUserHomepageFunc = async (row) => {
    const res = await deleteHkRecordBrowsingUserHomepage({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
