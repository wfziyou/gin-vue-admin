  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateHkCircleApplyGroupFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'HkCircleApplyGroup'
}
</script>

<script setup>
import {
  createHkCircleApplyGroup,
  deleteHkCircleApplyGroup,
  deleteHkCircleApplyGroupByIds,
  updateHkCircleApplyGroup,
  findHkCircleApplyGroup,
  getHkCircleApplyGroupList
} from '@/api/hkCircleApplyGroup'
...
const getTableData = async() => {
  const table = await getHkCircleApplyGroupList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkCircleApplyGroupFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkCircleApplyGroupByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkCircleApplyGroupFunc = async(row) => {
    const res = await findHkCircleApplyGroup({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkCircleApplyGroup
...
// 删除行
const deleteHkCircleApplyGroupFunc = async (row) => {
    const res = await deleteHkCircleApplyGroup({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkCircleApplyGroup(formData.value)
                  break
                case 'update':
                  res = await updateHkCircleApplyGroup(formData.value)
                  break
                default:
                  res = await createHkCircleApplyGroup(formData.value)
                  break
