<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="活动_编号" prop="activityId" width="120" />
        <el-table-column align="left" label="申请理由" prop="reason" width="120" />
        ...
        <el-table-column align="left" label="审核状态：0 未处理、1通过、2驳回" prop="checkStatus" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHkActivityAddRequestFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="活动_编号:"  prop="activityId" >
        ...
        <el-form-item label="审核状态：0 未处理、1通过、2驳回:"  prop="checkStatus" >
...
<script>
export default {
  name: 'HkActivityAddRequest'
}
</script>

<script setup>
import {
  createHkActivityAddRequest,
  deleteHkActivityAddRequest,
  deleteHkActivityAddRequestByIds,
  updateHkActivityAddRequest,
  findHkActivityAddRequest,
  getHkActivityAddRequestList
} from '@/api/hkActivityAddRequest'
...
const getTableData = async() => {
  const table = await getHkActivityAddRequestList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHkActivityAddRequestFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHkActivityAddRequestByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHkActivityAddRequestFunc = async(row) => {
    const res = await findHkActivityAddRequest({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehkActivityAddRequest
...
// 删除行
const deleteHkActivityAddRequestFunc = async (row) => {
    const res = await deleteHkActivityAddRequest({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createHkActivityAddRequest(formData.value)
                  break
                case 'update':
                  res = await updateHkActivityAddRequest(formData.value)
                  break
                default:
                  res = await createHkActivityAddRequest(formData.value)
                  break
